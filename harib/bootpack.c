...
void HariMain(void)
{
    ...
    mx = (binfo->scrnx - 16) / 2;	/* 화면 중앙 좌표 계산 */
        ...
        if (fifo32_status(&fifo) == 0) {
            ...
            if (256 <= i && i <= 511) {
                ...
                if (i == 256 + 0x0e) { // back space
                    if (key_to == 0) { // to task A
                        if (cursor_x > 8) { // if delete character and move cursor backward
                            putfonts8_asc_sht(sht_win, cursor_x, 28, COL8_000000,
                                    COL8_FFFFFF, " ", 1);
                            cursor_x -= 8;
                        }
                ...
                if (i == 256 + 0xfa) {  // keyboard data accepted
                    keycmd_wait = -1;
                }
                if (i == 256 + 0xfe) {  // keyboard data not accepted
                    wait_KBC_sendready();
                    io_out8(PORT_KEYDAT, keycmd_wait);
                }
                if (cursor_c >= 0) {
        ...
        if (fifo32_status(&task->fifo) == 0) {
            ...
            if (256 <= i && i <= 511) { // key board data
                if (i == 8 + 256) { // back space
                    ...
                    // Enter
                    if (cursor_y < 28 + 112) {  // remove cursor use space
